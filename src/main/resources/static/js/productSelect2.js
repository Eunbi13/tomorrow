...
function addList(){
  ...
  let unitPriceNumber //옵션가격
    = $("#selectop option:selected").attr("data-OPprice");
		...
		//2.optionAdd 안의 html를 가져와 opPrint에 넣는다.
...
// 바로결제 버튼 누를시 
//cart insert 먼저 해주기
// 그 후 order insert 바로 진행
function directPay(){

 // 바로결제 버튼 클릭시, 아무 옵션도 선택 안했다면 경고창! 
...
//DB에 validity가 20(바로결제 상태)인 cartVO를 다 지워주기
	       $.ajax({
				type:"get",
				url:"../cart/directPayDelete",
				data:{},
				success:function(data){
				//submit해서 cart/insert 시행
					 $("#frm").submit();
				}
			});
	   }
 ...
 // 장바구니버튼 클릭시 이벤트 
  $("#cartInsertBtn").click(function(){
	   ...
	   // 장바구니 버튼 클릭시, 아무 옵션도 선택 안했다면 경고창!
	  let source=$("#opPrint").html().trim();
	  if(source==null || source==""){
		  alert("옵션 선택 후에 버튼을 클릭해 주세요.");
	  }else{
