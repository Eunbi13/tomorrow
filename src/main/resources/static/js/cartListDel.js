...
$("#carts").on("click", "#selectedDelete", function(){
	...
	$(".del").each(function(){
		...
		if(ch){
			// 체크된 productNum를 배열에 추가
			ar.push($(this).attr("title")); 
		     //체크된 productNum의 brand를 배열에 추가
			brand_ar.push($(this).parent().attr("title"));
		}
	});
	...
	//해당 brand에 가지고 있는 다른 상품이 있는지 찾아 보고 없으면 brand 카드까지 삭제
						         ...
						         $(".brandPrice").each(function(){
								    ...
								    if(brandPrice>=30000){//한 브랜드에서 구매한게 3만원 이상
								    	$(".brandShipping"+b_index).each(function(){
								    		$(this).val(0);//cartVO brandShipping에 0 넣기 반복
								    	});
								    	...
								    	//2.그 브랜드의 어떤 productVO shippingFee도 0이 아님  ->false유지
									    	$(".shippingFee"+b_index).each(function(){ //+ checked된 것들중..
								    	...
								    	if(shipCheck==true){
								    		...
								    		let shipFee=100000; //shipFee에 가장 작은 배송비를 넣을 것 
									    			 ...
									    			 let productNum= $(this).attr("data-productNum"); // 그 브랜드안의 상품 넘버 반복
										    		 	for(let p in checkedNum){
										    		 		   console.log("checkedNum[p]:"+checkedNum[p]+"productNum:"+productNum);
											    			  	if(checkedNum[p] == productNum){ // checked productNum과 이 넘버가 같으면  
									    	   ...
									    	   if(shipFee==100000){//근데 하나도 같은 넘버가 없어서 if안에 한번도 못들어 갔으면 0 넣어주기
									    		   shipFee=0;
									    	   }
												
								    		$(".brandShipping"+b_index).each(function(){
									    		$(this).val(shipFee);//cartVO brandShipping에 찾은 가장 작은 배송비 넣기 반복
									    	});
